package ies

import (
	"github.com/lvdund/asn1go/aper"
	"github.com/lvdund/rrc/utils"
)

type SRS_PosResource_r16_resourceMapping_r16 struct {
	StartPosition_r16 int64                                                   `lb:0,ub:13,madatory`
	NrofSymbols_r16   SRS_PosResource_r16_resourceMapping_r16_nrofSymbols_r16 `madatory`
}

func (ie *SRS_PosResource_r16_resourceMapping_r16) Encode(w *aper.AperWriter) error {
	var err error
	if err = w.WriteInteger(ie.StartPosition_r16, &aper.Constraint{Lb: 0, Ub: 13}, false); err != nil {
		return utils.WrapError("WriteInteger StartPosition_r16", err)
	}
	if err = ie.NrofSymbols_r16.Encode(w); err != nil {
		return utils.WrapError("Encode NrofSymbols_r16", err)
	}
	return nil
}

func (ie *SRS_PosResource_r16_resourceMapping_r16) Decode(r *aper.AperReader) error {
	var err error
	var tmp_int_StartPosition_r16 int64
	if tmp_int_StartPosition_r16, err = r.ReadInteger(&aper.Constraint{Lb: 0, Ub: 13}, false); err != nil {
		return utils.WrapError("ReadInteger StartPosition_r16", err)
	}
	ie.StartPosition_r16 = tmp_int_StartPosition_r16
	if err = ie.NrofSymbols_r16.Decode(r); err != nil {
		return utils.WrapError("Decode NrofSymbols_r16", err)
	}
	return nil
}
