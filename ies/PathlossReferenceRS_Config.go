...
const (
	PathlossReferenceRS_Config_Choice_nothing uint64 = iota
	PathlossReferenceRS_Config_Choice_Ssb_Index
	PathlossReferenceRS_Config_Choice_Csi_RS_Index
)
...
func (ie *PathlossReferenceRS_Config) Encode(w *uper.UperWriter) error {
	var err error
	if err = w.WriteChoice(ie.Choice, 2, false); err != nil {
		return err
	}
	switch ie.Choice {
	case PathlossReferenceRS_Config_Choice_Ssb_Index:
		if err = ie.Ssb_Index.Encode(w); err != nil {
			err = utils.WrapError("Encode Ssb_Index", err)
		}
	case PathlossReferenceRS_Config_Choice_Csi_RS_Index:
...
func (ie *PathlossReferenceRS_Config) Decode(r *uper.UperReader) error {
	var err error
