...
type ReestabNCellInfo struct {
	...
	Key_gNodeB_Star aper.BitString `lb:256,ub:256,madatory`
	ShortMAC_I      ShortMAC_I     `madatory`
}

func (ie *ReestabNCellInfo) Encode(w *aper.AperWriter) error {
	var err error
	if err = ie.CellIdentity.Encode(w); err != nil {
		return utils.WrapError("Encode CellIdentity", err)
	}
	if err = w.WriteBitString(ie.Key_gNodeB_Star.Bytes, uint(ie.Key_gNodeB_Star.NumBits), &aper.Constraint{Lb: 256, Ub: 256}, false); err != nil {
...
func (ie *ReestabNCellInfo) Decode(r *aper.AperReader) error {
	var err error
	...
	if tmp_bs_Key_gNodeB_Star, n_Key_gNodeB_Star, err = r.ReadBitString(&aper.Constraint{Lb: 256, Ub: 256}, false); err != nil {
		return utils.WrapError("ReadBitString Key_gNodeB_Star", err)
	}
	ie.Key_gNodeB_Star = aper.BitString{
