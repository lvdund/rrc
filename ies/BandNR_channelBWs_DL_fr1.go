...
type BandNR_channelBWs_DL_fr1 struct {
	Scs_15kHz *aper.BitString `lb:10,ub:10,optional`
	Scs_30kHz *aper.BitString `lb:10,ub:10,optional`
	Scs_60kHz *aper.BitString `lb:10,ub:10,optional`
}

func (ie *BandNR_channelBWs_DL_fr1) Encode(w *aper.AperWriter) error {
	var err error
	...
	if ie.Scs_15kHz != nil {
		if err = w.WriteBitString(ie.Scs_15kHz.Bytes, uint(ie.Scs_15kHz.NumBits), &aper.Constraint{Lb: 10, Ub: 10}, false); err != nil {
			return utils.WrapError("Encode Scs_15kHz", err)
		}
	}
	if ie.Scs_30kHz != nil {
		if err = w.WriteBitString(ie.Scs_30kHz.Bytes, uint(ie.Scs_30kHz.NumBits), &aper.Constraint{Lb: 10, Ub: 10}, false); err != nil {
			return utils.WrapError("Encode Scs_30kHz", err)
		}
	}
	if ie.Scs_60kHz != nil {
		if err = w.WriteBitString(ie.Scs_60kHz.Bytes, uint(ie.Scs_60kHz.NumBits), &aper.Constraint{Lb: 10, Ub: 10}, false); err != nil {
...
func (ie *BandNR_channelBWs_DL_fr1) Decode(r *aper.AperReader) error {
	var err error
	...
	if Scs_15kHzPresent {
		...
		if tmp_bs_Scs_15kHz, n_Scs_15kHz, err = r.ReadBitString(&aper.Constraint{Lb: 10, Ub: 10}, false); err != nil {
	...
	if Scs_30kHzPresent {
		...
		if tmp_bs_Scs_30kHz, n_Scs_30kHz, err = r.ReadBitString(&aper.Constraint{Lb: 10, Ub: 10}, false); err != nil {
	...
	if Scs_60kHzPresent {
		...
		if tmp_bs_Scs_60kHz, n_Scs_60kHz, err = r.ReadBitString(&aper.Constraint{Lb: 10, Ub: 10}, false); err != nil {
