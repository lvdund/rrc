...
func (ie *DummyF) Encode(w *aper.AperWriter) error {
	var err error
	if err = w.WriteInteger(ie.MaxNumberPeriodicCSI_ReportPerBWP, &aper.Constraint{Lb: 1, Ub: 4}, false); err != nil {
		return utils.WrapError("WriteInteger MaxNumberPeriodicCSI_ReportPerBWP", err)
	}
	if err = w.WriteInteger(ie.MaxNumberAperiodicCSI_ReportPerBWP, &aper.Constraint{Lb: 1, Ub: 4}, false); err != nil {
		return utils.WrapError("WriteInteger MaxNumberAperiodicCSI_ReportPerBWP", err)
	}
	if err = w.WriteInteger(ie.MaxNumberSemiPersistentCSI_ReportPerBWP, &aper.Constraint{Lb: 0, Ub: 4}, false); err != nil {
...
func (ie *DummyF) Decode(r *aper.AperReader) error {
	var err error
	var tmp_int_MaxNumberPeriodicCSI_ReportPerBWP int64
	if tmp_int_MaxNumberPeriodicCSI_ReportPerBWP, err = r.ReadInteger(&aper.Constraint{Lb: 1, Ub: 4}, false); err != nil {
	...
	if tmp_int_MaxNumberAperiodicCSI_ReportPerBWP, err = r.ReadInteger(&aper.Constraint{Lb: 1, Ub: 4}, false); err != nil {
	...
	if tmp_int_MaxNumberSemiPersistentCSI_ReportPerBWP, err = r.ReadInteger(&aper.Constraint{Lb: 0, Ub: 4}, false); err != nil {
