...
type RRCResumeRequest1_IEs struct {
	...
	ResumeMAC_I    aper.BitString `lb:16,ub:16,madatory`
	ResumeCause    ResumeCause    `madatory`
	Spare          aper.BitString `lb:1,ub:1,madatory`
}

func (ie *RRCResumeRequest1_IEs) Encode(w *aper.AperWriter) error {
	var err error
	if err = ie.ResumeIdentity.Encode(w); err != nil {
		return utils.WrapError("Encode ResumeIdentity", err)
	}
	if err = w.WriteBitString(ie.ResumeMAC_I.Bytes, uint(ie.ResumeMAC_I.NumBits), &aper.Constraint{Lb: 16, Ub: 16}, false); err != nil {
...
func (ie *RRCResumeRequest1_IEs) Decode(r *aper.AperReader) error {
	var err error
	...
	if tmp_bs_ResumeMAC_I, n_ResumeMAC_I, err = r.ReadBitString(&aper.Constraint{Lb: 16, Ub: 16}, false); err != nil {
		return utils.WrapError("ReadBitString ResumeMAC_I", err)
	}
	ie.ResumeMAC_I = aper.BitString{
