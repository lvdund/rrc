...
type CodebookConfig_codebookType_type1_subType_typeI_MultiPanel struct {
	...
	Ri_Restriction aper.BitString                                                      `lb:4,ub:4,madatory`
}

func (ie *CodebookConfig_codebookType_type1_subType_typeI_MultiPanel) Encode(w *aper.AperWriter) error {
	var err error
	if err = ie.Ng_n1_n2.Encode(w); err != nil {
		return utils.WrapError("Encode Ng_n1_n2", err)
	}
	if err = w.WriteBitString(ie.Ri_Restriction.Bytes, uint(ie.Ri_Restriction.NumBits), &aper.Constraint{Lb: 4, Ub: 4}, false); err != nil {
		return utils.WrapError("WriteBitString Ri_Restriction", err)
	}
	return nil
}

func (ie *CodebookConfig_codebookType_type1_subType_typeI_MultiPanel) Decode(r *aper.AperReader) error {
	var err error
	...
	if tmp_bs_Ri_Restriction, n_Ri_Restriction, err = r.ReadBitString(&aper.Constraint{Lb: 4, Ub: 4}, false); err != nil {
		return utils.WrapError("ReadBitString Ri_Restriction", err)
	}
	ie.Ri_Restriction = aper.BitString{
