...
func (ie *SupportedCSI_RS_Resource) Encode(w *aper.AperWriter) error {
	var err error
	if err = ie.MaxNumberTxPortsPerResource.Encode(w); err != nil {
		return utils.WrapError("Encode MaxNumberTxPortsPerResource", err)
	}
	if err = w.WriteInteger(ie.MaxNumberResourcesPerBand, &aper.Constraint{Lb: 1, Ub: 64}, false); err != nil {
		return utils.WrapError("WriteInteger MaxNumberResourcesPerBand", err)
	}
	if err = w.WriteInteger(ie.TotalNumberTxPortsPerBand, &aper.Constraint{Lb: 2, Ub: 256}, false); err != nil {
		return utils.WrapError("WriteInteger TotalNumberTxPortsPerBand", err)
	}
	return nil
}

func (ie *SupportedCSI_RS_Resource) Decode(r *aper.AperReader) error {
	var err error
	...
	if tmp_int_MaxNumberResourcesPerBand, err = r.ReadInteger(&aper.Constraint{Lb: 1, Ub: 64}, false); err != nil {
	...
	if tmp_int_TotalNumberTxPortsPerBand, err = r.ReadInteger(&aper.Constraint{Lb: 2, Ub: 256}, false); err != nil {
