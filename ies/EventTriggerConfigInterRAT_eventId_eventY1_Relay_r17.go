...
type EventTriggerConfigInterRAT_eventId_eventY1_Relay_r17 struct {
	Y1_Threshold1_r17       MeasTriggerQuantity        `madatory`
	Y1_Threshold2_Relay_r17 SL_MeasTriggerQuantity_r16 `madatory`
	ReportOnLeave_r17       bool                       `madatory`
	Hysteresis_r17          Hysteresis                 `madatory`
	TimeToTrigger_r17       TimeToTrigger              `madatory`
}

func (ie *EventTriggerConfigInterRAT_eventId_eventY1_Relay_r17) Encode(w *uper.UperWriter) error {
	var err error
	if err = ie.Y1_Threshold1_r17.Encode(w); err != nil {
		return utils.WrapError("Encode Y1_Threshold1_r17", err)
	}
	if err = ie.Y1_Threshold2_Relay_r17.Encode(w); err != nil {
		return utils.WrapError("Encode Y1_Threshold2_Relay_r17", err)
	}
...
func (ie *EventTriggerConfigInterRAT_eventId_eventY1_Relay_r17) Decode(r *uper.UperReader) error {
	var err error
	if err = ie.Y1_Threshold1_r17.Decode(r); err != nil {
		return utils.WrapError("Decode Y1_Threshold1_r17", err)
	}
	if err = ie.Y1_Threshold2_Relay_r17.Decode(r); err != nil {
		return utils.WrapError("Decode Y1_Threshold2_Relay_r17", err)
	}
