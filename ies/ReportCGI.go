...
type ReportCGI struct {
	...
	UseAutonomousGaps_r16   *ReportCGI_useAutonomousGaps_r16 `optional,ext-1`
}

func (ie *ReportCGI) Encode(w *aper.AperWriter) error {
	var err error
	...
	if hasExtensions {
		...
		if err := w.WriteExtBitMap(extBitmap); err != nil {
			return utils.WrapError("WriteExtBitMap ReportCGI", err)
		}
...
func (ie *ReportCGI) Decode(r *aper.AperReader) error {
	var err error
	...
	if extensionBit {
		...
		if len(extBitmap) > 0 && extBitmap[0] {
			...
			if UseAutonomousGaps_r16Present {
				ie.UseAutonomousGaps_r16 = new(ReportCGI_useAutonomousGaps_r16)
