...
type PH_InfoMCG struct {
	...
	Ph_Uplink                   PH_UplinkCarrierMCG                     `madatory`
	Ph_SupplementaryUplink      *PH_UplinkCarrierMCG                    `optional`
...
func (ie *PH_InfoMCG) Encode(w *aper.AperWriter) error {
	...
	if hasExtensions {
		...
		if err := w.WriteExtBitMap(extBitmap); err != nil {
			return utils.WrapError("WriteExtBitMap PH_InfoMCG", err)
		}
...
func (ie *PH_InfoMCG) Decode(r *aper.AperReader) error {
	...
	if Ph_SupplementaryUplinkPresent {
		ie.Ph_SupplementaryUplink = new(PH_UplinkCarrierMCG)
	...
	if extensionBit {
		...
		if len(extBitmap) > 0 && extBitmap[0] {
			...
			if TwoSRS_PUSCH_Repetition_r17Present {
				ie.TwoSRS_PUSCH_Repetition_r17 = new(PH_InfoMCG_twoSRS_PUSCH_Repetition_r17)
