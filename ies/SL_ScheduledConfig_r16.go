...
func (ie *SL_ScheduledConfig_r16) Encode(w *aper.AperWriter) error {
	var err error
	...
	if len(ie.Sl_PSFCH_ToPUCCH_r16) > 0 {
		tmp_Sl_PSFCH_ToPUCCH_r16 := utils.NewSequence[*utils.INTEGER]([]*utils.INTEGER{}, aper.Constraint{Lb: 1, Ub: 8}, false)
	...
	if hasExtensions {
		...
		if extBitmap[0] {
			...
			if len(ie.Sl_DCI_ToSL_Trans_r16) > 0 {
				tmp_Sl_DCI_ToSL_Trans_r16 := utils.NewSequence[*utils.INTEGER]([]*utils.INTEGER{}, aper.Constraint{Lb: 1, Ub: 8}, false)
				for _, i := range ie.Sl_DCI_ToSL_Trans_r16 {
					tmp_ie := utils.NewINTEGER(int64(i), aper.Constraint{Lb: 1, Ub: 32}, false)
...
func (ie *SL_ScheduledConfig_r16) Decode(r *aper.AperReader) error {
	var err error
		...
		if len(extBitmap) > 0 && extBitmap[0] {
			...
			if Sl_DCI_ToSL_Trans_r16Present {
				...
				fn_Sl_DCI_ToSL_Trans_r16 := func() *utils.INTEGER {
					ie := utils.NewINTEGER(0, aper.Constraint{Lb: 1, Ub: 32}, false)
					return &ie
				}
