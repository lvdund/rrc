...
func (ie *CSI_RS_ForTracking) Encode(w *aper.AperWriter) error {
	var err error
	if err = w.WriteInteger(ie.MaxBurstLength, &aper.Constraint{Lb: 1, Ub: 2}, false); err != nil {
		return utils.WrapError("WriteInteger MaxBurstLength", err)
	}
	if err = w.WriteInteger(ie.MaxSimultaneousResourceSetsPerCC, &aper.Constraint{Lb: 1, Ub: 8}, false); err != nil {
		return utils.WrapError("WriteInteger MaxSimultaneousResourceSetsPerCC", err)
	}
	if err = w.WriteInteger(ie.MaxConfiguredResourceSetsPerCC, &aper.Constraint{Lb: 1, Ub: 64}, false); err != nil {
		return utils.WrapError("WriteInteger MaxConfiguredResourceSetsPerCC", err)
	}
	if err = w.WriteInteger(ie.MaxConfiguredResourceSetsAllCC, &aper.Constraint{Lb: 1, Ub: 256}, false); err != nil {
		return utils.WrapError("WriteInteger MaxConfiguredResourceSetsAllCC", err)
	}
	return nil
}

func (ie *CSI_RS_ForTracking) Decode(r *aper.AperReader) error {
	var err error
	var tmp_int_MaxBurstLength int64
	if tmp_int_MaxBurstLength, err = r.ReadInteger(&aper.Constraint{Lb: 1, Ub: 2}, false); err != nil {
	...
	if tmp_int_MaxSimultaneousResourceSetsPerCC, err = r.ReadInteger(&aper.Constraint{Lb: 1, Ub: 8}, false); err != nil {
	...
	if tmp_int_MaxConfiguredResourceSetsPerCC, err = r.ReadInteger(&aper.Constraint{Lb: 1, Ub: 64}, false); err != nil {
	...
	if tmp_int_MaxConfiguredResourceSetsAllCC, err = r.ReadInteger(&aper.Constraint{Lb: 1, Ub: 256}, false); err != nil {
